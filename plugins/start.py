    ...
    if fsub_data:
        ...
        if must_join:
            ...
            if buttons:
                ...
                buttons.append([InlineKeyboardButton("🔄 Try Again", url=f"https://t.me/{me.username}?start={start_param}")])
                return await message.reply_text(
                    "<b>👋 Welcome! To MD Links Share Bot.\n\nTo use this bot, you must join our channels first. Click the buttons below to join, then click 'Try Again'.</b>",
...
async def check_subscription_status(client: Client, user_id: int, fsub_channels: list):
    ...
    message_text = "<b>👋 Welcome! To MD Links Share Bot, To use this bot, you must join our channels first. Click the buttons below to join, then click 'Try Again'.</b>"
